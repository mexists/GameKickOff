...
class JoshController extends Controller {
	...
	public function __construct() {
		...
		$this->countries = [];
		foreach (Country::where('name', '=', 'Malaysia')->get() as $country)
			$this->countries[explode('/', $country->iso)[0]] = $country->name;

		$this->countries = array_merge(['' => 'Select Country'], $this->countries);
	...
	public function generateOTP() { // generate One-Time Password
		$otp = null;
		do {
			$string = 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
			$string_shuffled = str_shuffle($string);
			$password = substr($string_shuffled, 1, 4);
			$otp = str_replace("=", "", base64_encode($password));
		} while (Phone::where('otp', '=', $otp)->exists());
