...
class FrontEndController extends JoshController {
	...
	public function getRegister() {
		...
		$urlResetNumber = preg_replace("@^https?://@", "", url("reset"));
		if (!Session::has('confirmPhoneNumber'))
			return redirect('confirm')->with('error', trans("Please enter your OTP or reset it <a href='$urlResetNumber'>here</a>."));

		$confirmPhoneNumber = Session::get('confirmPhoneNumber');
		// Show the page
		return View::make('register', compact('confirmPhoneNumber'));
	}
	...
	public function postRegister(UserRequest $request) {
		...
		try {
			// Register the user
			$user = Sentinel::register($request->except(['_token', 'password_confirm', 'terms_and_conditions', 'submit']), $activate);

			$phone = Phone::where('number', '=', Session::get('confirmPhoneNumber'))->orderBy('id')->first();
			$phone->user_id = $user->id;
			$phone->phone_status_id = 1;
			$phone->save();
			if (Session::has('confirmPhoneNumber'))
				Session::forget('confirmPhoneNumber');
	...
	public function getTokens() {
		$TokenPackages = TokenPackage::latest()->get();
		if (\Input::has('id')) {
			$id = \Input::get('id');
			$TokenPackage = TokenPackage::findOrFail($id);
			Cart::destroy();
			Cart::add($TokenPackage->id, $TokenPackage->title, 1, $TokenPackage->price);

			// Redirect to the checkout page
			return Redirect::to('payment/checkout');
		}

		return View::make('tokens', compact('TokenPackages'));
	}
